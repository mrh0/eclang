...
class Module private constructor(val namespace: String) {
    ...
    val globalContext = ModuleContextBuilder(namespace)

    fun addOverride(identifier: String, override: OverrideTemplate) {
        if (nameCollisionSet.contains(override.sourceName)) { throw EcNameCollisionError(
            override.location,
            override.sourceName
        )
        }
    ...
    fun getOverride(location: Loc, identifier: String, arguments: Array<EcType>): OverrideTemplate {
        val template = functions.getOrElse(Util.testIdentifier(location, identifier)) { throw EcFunctionNotDefinedError(
            location,
            identifier,
            namespace
        )
