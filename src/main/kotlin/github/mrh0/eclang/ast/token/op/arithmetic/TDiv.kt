...
import github.mrh0.eclang.ir.arithmetic.div.IRDiv
...
class TDiv(location: Loc, val left: ITok, val right: ITok) : Tok(location) {
    ...
    override fun process(cd: CompileData, hint: EcType): Pair<EcType, IIR> {
        ...
        return when {
            l.first is EcTypeInt || r.first is EcTypeInt -> Pair(EcTypeInt, IRDiv(location, l.second, r.second))
            l.first is EcTypeNumber || r.first is EcTypeNumber -> Pair(l.first, IRDiv(location, l.second, r.second))
