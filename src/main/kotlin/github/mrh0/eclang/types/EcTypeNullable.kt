package github.mrh0.eclang.types
/*
import github.mrh0.eclang.ast.Loc
import github.mrh0.eclang.error.EcNotReferenceTypeError

class EcTypeNullable private constructor(val wrapped: EcType) : EcType(getNullableId(wrapped), EcTypeUnion.of(wrapped, EcTypeNull)) {
    companion object {
        private fun getNullableId(wrapped: EcType) = "Nullable($wrapped)"

        fun of(wrapped: EcType): EcType = EcTypeUnion.of(wrapped, EcTypeNull)//ALL_TYPES.getOrPut(getNullableId(wrapped)) { EcTypeNullable(wrapped) }
    }

    //override fun accepts(location: Loc, type: EcType): Boolean = if (type is EcTypeNullable) wrapped.accepts(location, type.wrapped) else if(type is EcTypeNull) true else wrapped.accepts(location, type)

    override fun isReferenceType(): Boolean = wrapped.isReferenceType() // nullable must be ref type
}
*/
